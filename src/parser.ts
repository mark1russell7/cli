...
import type { CLISpec, CommandSpec, ParsedArgs } from "./types.js";
 ...
 */
export function buildCLI(spec: CLISpec): Command {
 ...
 */
function addCommand(parent: Command, spec: CommandSpec): void {
  ...
  // Add action handler
  leafCmd.action(async (...actionArgs: unknown[]) => {
    ...
    const rawOptions = actionArgs[optionsIndex] as Record<string, unknown> | undefined;
